...
A Cluster whose members are not temporally contiguous this way — some other photo's timestamp falls strictly between its earliest and latest member — shows every one of its own timestamps in red instead of the normal muted color, with an explanatory `title` so the signal isn't color-only. In practice this usually means one or more of those timestamps are wrong (a common cause: a re-export or forwarding step resetting the capture time), since a similarity match with a real temporal gap in the middle is otherwise unexpected. A tie at the exact boundary (an outside photo's timestamp exactly equal to the Cluster's own earliest or latest) does not count as breaking contiguity — only a strictly-between timestamp does.
