...
- KTD9. **`appendWithIndex` (`hooks/usePhotos.ts`, backing U6's append fix and the pre-existing `addPhotos`) renumbers `prev`'s `uploadIndex` to its current array position before appending new entries.** Code review found the prior version carried forward each photo's original `uploadIndex`, which `sortPhotos` uses as a tiebreaker for photos with no EXIF date. Dragging an undated photo among other undated ones only changes array position (`slotTimestamp`'s all-null branch leaves `capturedAt` untouched), so a later `processFiles`/`addPhotos` append re-sorted by the stale `uploadIndex` and silently reverted the drag. Renumbering keeps the tiebreaker consistent with what's on screen.
