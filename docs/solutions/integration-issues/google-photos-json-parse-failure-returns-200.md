...
- `docs/solutions/integration-issues/google-photos-picker-pagination-token-never-followed.md` — another related but distinct hazard in the same media-items fetch and the same `app/api/google-photos/sessions/[id]/route.ts` GET handler: a missing continuation-loop across separate calls (the code never followed `nextPageToken`) rather than this doc's same-call JSON-parse-laundering bug.
